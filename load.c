...
/*   Updated: 2025/09/12 20:41:23 by thcaquet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		while (++j <= 3  && ++p)
		{
			*p = ft_atoi(&line[i]);
			...
			if (*p >= 0 && *p <= 255
				...
				tool_exit(1, "(1)" ERROR_RGB, data, line); // tmp
		}
	}
	else
		tool_exit(1, "(2)" ERROR_RGB, data, line); // tmp
	i += ft_strcount(&line[i - 1], ' ');
	if (line[i - 1] != '\n')
		tool_exit(1, "(3)" ERROR_RGB, data, line); // tmp
}
...
void	load_tex(char *line, int len, t_data *data, int cardinal)
{
	...
	if (data->tex[cardinal] == 0)
	{
		...
		tool_exit(1, ERROR_MULTI_TEX, data, line);
	if (data->tex[cardinal] == 0)
		tool_exit(1, ERROR_LOAD_TEX, data, line);
}
...
/*   Updated: 2025/10/23 15:01:56 by thcaquet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_tex_door(t_data *data, t_door *door, t_pointi i, \
const float tex_step)
{
	t_draw_tex	draw;
	int			offset;
	int			tex_x;
	...
	offset = ((float)data->map[1][door->map.y][door->map.x] / 250.0f) \
* draw.tex_w;
	draw.tex_pos = (draw.start - HEIGHT / 2.0 + door->line_height / 2.0) \
* tex_step;
	while (draw.start < draw.end)
	{
		...
		if (!(draw.tex_x < draw.dst + offset && draw.tex_x > draw.dst - offset\
			...
			data->buf[draw.start * WIDTH + i.x] = \
door->tex->buf[tex_x + draw.tex_y * draw.tex_w];
...
{
	draw_tex_door(data, door, i, \
(float)door->tex->height / (float)door->line_height);
}

...
/*   Updated: 2025/10/09 03:15:25 by jaineko          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_draw_tex	init_draw_tex(t_data *data, t_dda *dda)
{
	...
	draw.color.x = tool_rgba(data->rgb[C].r, data->rgb[C].g, data->rgb[C].b, 255);
	draw.color.y = tool_rgba(data->rgb[F].r, data->rgb[F].g, data->rgb[F].b, 255);
...
void	draw_tex(t_data *data, t_dda *dda, t_pointi i, const float tex_step)
{
	...
	draw.tex_pos = (draw.start - HEIGHT / 2.0 + dda->line_height / 2.0) * tex_step;
	i.y = -1;
	...
	while (i.y < draw.end)
	{
		...
		if (draw.tex_y >= draw.tex_h)
			break ;
		if (draw.tex_y > 0)
			data->buf[i.y * WIDTH + i.x] = dda->tex->buf[draw.dst + draw.tex_y * draw.tex_w];
